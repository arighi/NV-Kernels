 ...
 * two-thread active mode, with resources partitioned between the PEs. Prefer
 * PE0 so PE1 can remain idle and the core can stay in full-resource mode.
 * Firmware does not currently describe this preference, so detect Olympus by
 * MIDR until a firmware interface is available.
 */
